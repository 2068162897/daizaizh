...
function BrandMark() {
  return (
    <svg viewBox="0 0 24 24" className="w-4 h-4 inline-block align-middle" fill="none" stroke="currentColor" strokeWidth="2">
...
export default function Navbar() {
  ...
  return (
    <header
      className={`w-full fixed top-0 left-0 right-0 z-50 transition-all duration-500 border-b ${
            ...
            return (
              <Link
                ...
                className={`relative py-1 transition-colors ${
                  isActive ? "text-indigo-600 dark:text-indigo-400" : "text-slate-700 dark:text-slate-200 hover:text-indigo-600"
                }`}
              >
                {link.name}
                {isActive && (
